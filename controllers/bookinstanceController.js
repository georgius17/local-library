...
exports.bookinstance_update_get = function(req, res) {
    BookInstance.findById(req.params.id).populate('book').exec((err, result) => {
        ...
        res.render('bookinstance_form', {title: 'Update BookInstance', book_list: result.book.title, bookinstance: result});
    })
};
        ...
        if (!errors.isEmpty()) {
            ...
            BookInstance.findById(req.params.id).populate('book')
                .exec(function (err, result) {
                    if (err) { return next(err); }
                    // Successful, so render.
                    res.render('bookinstance_form', { title: 'Update BookInstance', book_list: result.book.title, selected_book: result.book._id , errors: errors.array(), bookinstance: result });
            });
            return;
        }
        else {
